...
#write.table(Gene_Row_Key, file = 'Trans_v_Untrans_WUS_SBC_Gene_Row_Key.tsv', sep='\t', quote=FALSE, row.names = F)
rm(x)
...
simplified_DGE = decideTestsDGE(Fish_Exact, p=0.05, adjust="BH")
my_test = rownames(Fish_Exact)[as.logical(simplified_DGE)]
jpeg('ALL_Ungalled_WUS_SBC_Smear.png')
plotSmear(Fish_Exact, de.tags=my_test, main = "ALL Ungalled WUS Knockdowns vs. SBC Control Smear Plot")
abline(h=c(0,10))
dev.off()
# Write summary table
write.table(summary(simplified_DGE), file = 'ALL_Ungalled_WUS_SBC_Summary.txt', quote = F, sep = '\t')
...
row.names(simplified_DGE_frame) = Gene_Row_Key$Gene
Trans_v_Untrans_Wus_SBC_Direction = as_tibble(rownames_to_column(simplified_DGE_frame, var = 'Gene_Name'))
write_tsv(Trans_v_Untrans_Wus_SBC_Direction, 'ALL_Ungalled_WUS_SBC_Direction.tsv')

# Write full output
row.names(Fish_Exact) = Gene_Row_Key$Gene
Fish_tbl = as_tibble(rownames_to_column(Fish_Exact$table, var = 'Gene_Name'))
write_tsv(Fish_tbl, 'ALL_ungalled_WUS_SBC_Full.tsv')

# Clear workspace
rm(Gene_Row_Key, D, D_Samples, Fish_Exact, my_grouping, Counts, topTags, WUS_Transformed_V_Untransformed)
```

## Transformed_v_Untransformed_V2
Now we will work on the other section of untransformed data
```{r Transformed_v_Untransformed_V2, eval=FALSE}
# Switch to output folder
setwd('/home/scott/Documents/Uni/Research/Projects/Grape_RNA/Output')
#-----------------------------------
# Transformed vs. Untransformed
...
LFY_Transformed_V_Untransformed_No_Gall = LFY_Transformed_V_Untransformed_No_Gall(Complete_Set)
...
Gene_Row_Key = data.frame("Num"=rownames(as.data.frame(x)),"Gene"=x)
# Write the table if you want.
#write.table(Gene_Row_Key, file = 'Trans_v_Untrans_LFY_SBC_Gene_Row_Key.tsv', sep='\t', quote=FALSE, row.names = F)
rm(x)
...
simplified_DGE = decideTestsDGE(Fish_Exact, p=0.05, adjust="BH")
my_test = rownames(Fish_Exact)[as.logical(simplified_DGE)]
plotSmear(Fish_Exact, de.tags=my_test)
...
# Clear workspace
rm(Gene_Row_Key, D, D_Samples, Fish_Exact, my_grouping, Counts, topTags, LFY_Transformed_V_Untransformed_No_Gall)
```
...
Counts = merge.data.frame(DPR_EmptVec_With_Gall, DPR_EmptVec_No_Gall, by=0) # merge relevant data
...
Gene_Row_Key = data.frame("Num"=rownames(as.data.frame(x)),"Gene"=x)
# Decoder to match back the gene names to the numbers
write.table(Gene_Row_Key, file = 'Gene_Row_Key_DPR.tsv', sep='\t', quote=FALSE, row.names = F)
rm(x)
...
#TODO
## Below we are going to examine the  grouping. These are the empty vector controls with and without galls. 
NOTE: this comparison cannot be performed as there is no DPR no gall (DPR-x) sample. I will verify later.
```{r DPR_Self_2, eval = FALSE}
# Switch to output folder
setwd('/home/scott/Documents/Uni/Research/Projects/Grape_RNA/Output')
#-----------------------------------
# DPR Within Self
